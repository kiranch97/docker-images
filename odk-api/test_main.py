...
def test_get_detected_objects_no_stream_id():
    response = client.get("/detected_objects?day=1971-12-12%2023%3A23%3A00.0")
    assert response.status_code == 422

def test_get_detected_objects_no_day():
    response = client.get("/detected_objects?stream_id=1")
    assert response.status_code == 422

def test_get_detected_objects_invalid_data():
    response = client.get("/detected_objects?stream_id=1&day=1971-12-12%2023%3A23%3A00.0")
...
def test_last_analysed_frames():
    response = client.get("/last_analysed_frames?stream_id=1")
    assert response.status_code == 200
    assert response.json() == None

def test_dash_stream_devices():
    response = client.get("/dash_stream_devices?stream_id=1&day=1971-12-12%2023%3A23%3A00.0")
    assert response.status_code == 500
    assert response.json() == {"code": 2, "message": "Cannot get analysed frames. \
                                Problem with server!", "status": "error"}
