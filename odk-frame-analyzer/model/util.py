...
def process_result(detection, obj_threshhold, nms_threshhold):
    ...
    for batchi in range(detection.size(0)):
        ...
        # attributes of each bounding box: x1, y1, x2, y2, objectness score,
        # prediction score, prediction index
        ...
        for cls in pred_classes:
            # select boxes that predict the class
            bboxes_cls = bboxes[bboxes[:, -1] == cls]
            _, sort_indices = torch.sort(bboxes_cls[:, 4], descending=True)
            bboxes_cls = bboxes_cls[sort_indices]   # sort by objectness score

            # select the box with the highest score and get rid
            # of intercepting boxes with big IOU
            boxi = 0
            while boxi + 1 < bboxes_cls.size(0):
                ious = compute_ious(bboxes_cls[boxi], bboxes_cls[boxi+1:])
                bboxes_cls = torch.cat([
                    bboxes_cls[:boxi+1],
                    bboxes_cls[boxi+1:][ious < nms_threshhold]
                ])
                boxi += 1
...
def compute_ious(target_box, comp_boxes):
    ...
    intercept_areas = (
        torch.clamp(interceptx2s - interceptx1s + 1, 0)
        * torch.clamp(intercepty2s - intercepty1s + 1, 0)
    )
...
def resize_image(img, size):
    """Resize_image by scaling while preserving aspect ratio and then
    padding remaining area with gray pixels
    """
    h, w = img.shape[0:2]
    newh, neww = size
...
def transform_result(detections, imgs, input_size):
    """
    Transform bouning box position in the resized image(input image to the
    network) to the corresponding position in the original image
    """
    ...
    detections[:, [1, 3]] -= (
        input_size[1] - scale_factors * img_dims[:, 1].unsqueeze(-1)) / 2
    detections[:, [2, 4]] -= (
        input_size[0] - scale_factors * img_dims[:, 0].unsqueeze(-1)) / 2

    detections[:, 1:5] /= scale_factors

    # clipping
    detections[:, 1:5] = torch.clamp(detections[:, 1:5], 0)
    detections[:, [1, 3]] = torch.min(
        detections[:, [1, 3]], img_dims[:, 1].unsqueeze(-1))
    detections[:, [2, 4]] = torch.min(
        detections[:, [2, 4]], img_dims[:, 0].unsqueeze(-1))
